...
class TestRulePairsRealShape:
    ...
    pair added 2026-08-10, the rhdh-plugins pair added 2026-08-13, the
    kubernaut-docs pair added 2026-08-16, or the koku-service-operator pair
    added 2026-08-17 -- TestMain below only ever exercises fake pairs, so it
    wouldn't catch that regression on its own."""

    def test_all_nine_pairs_are_registered(self, check_rule_sync):
        assert set(check_rule_sync.RULE_PAIRS.keys()) == {"global", "engram", "operator", "console", "koku", "koku-service-operator", "praxis", "rhdh-plugins", "kubernaut-docs"}
