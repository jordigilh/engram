...
def _split_text(text: str, chunk_size: int = 800, chunk_overlap: int = 200) -> list[str]:
    ...
    Kept here as a thin alias to chunking.split_fixed_window() (still used
    directly by code_app below, which has no stable-heading concept and
    isn't routed through hindsight-api's delta-retain/consolidation
    pipeline). process_doc_file/process_issue now use
    chunking.split_markdown_sections()/split_issue_sections() instead --
