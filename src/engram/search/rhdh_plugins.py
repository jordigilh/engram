#!/usr/bin/env python3
r"""rhdh-plugins code search MCP server.

Provides hybrid code search (dense vectors + BM25) over the
cocoindex.rhdh_plugins_code_embeddings table. Results are fused using
Reciprocal Rank Fusion (RRF) so both semantic similarity and exact keyword
matches contribute to ranking.

Scoped to workspaces/boost/ only -- see rhdh_plugins.py flow module
docstring for why this project's onboarding narrows to that one workspace
package rather than the full 23-package monorepo.

Usage:
    python3 rhdh-plugins-cocoindex-search.py                    # Start MCP server (stdio)
...
_PATTERN_SEARCH_ROOTS = [
    ("rhdh-plugins-boost", RHDH_PLUGINS_BOOST_DIR, ["**/*.ts", "**/*.tsx"], [
        "**/node_modules/**", "**/dist/**", "**/dist-dynamic/**",
        "**/*.test.ts", "**/*.test.tsx", "**/coverage/**",
    ]),
]
...
def pattern_search_code(pattern: str, language: str, limit: int = 10) -> list[dict[str, Any]]:
    ...
    tree-sitter AST matching against the live rhdh-plugins checkout
...
def _run_mcp_server(host: str = "127.0.0.1", port: int = 8889, transport: str = "stdio") -> None:
    ...
    mcp = mcp_compat.make_server("rhdh-plugins-code", host=host, port=port)

    @mcp.tool()
    def rhdh_plugins_code_search(query: str, limit: int = 10) -> str:
        """Hybrid code search over the rhdh-plugins codebase (workspaces/boost/ only).
        ...
        - conceptual queries: "how does graduated visibility permission checking work?"
        - exact identifiers: "AiCatalogFilterBlueprint"
    ...
    @mcp.tool()
    def rhdh_plugins_code_pattern_search(pattern: str, language: str = "typescript", limit: int = 10) -> str:
        r"""Structural ("by-example") code search over the rhdh-plugins codebase
        (workspaces/boost/ only).

        For "find code shaped like X" -- e.g. every function matching a
        signature -- not "find code about X" (use rhdh_plugins_code_search
        for that). Matches by tree-sitter AST shape, not text/regex.
    ...
    if transport == "stdio":
        ...
        log.info("Starting rhdh-plugins-code MCP server on %s:%d (sse)", host, port)
...
def main():
    ...
    parser.add_argument("--language", default="typescript", help="Language for --pattern (default: typescript)")
