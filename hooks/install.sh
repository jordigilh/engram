...
#
# What this does:
#   1. Symlinks the hook scripts into the shared ~/.engram/hooks/
#      location (single source of truth, same pattern as chunking.py's
#      symlink into ~/.engram/ -- see docs/INSTALL.md step 16), so every
...
# ~/.engram/venv's Python (system python3 lacks litellm/vertexai).
set -euo pipefail

ENGRAM_HOOKS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TARGET_REPO="${1:-$(pwd)}"
SHARED_HOOKS_DIR="$HOME/.engram/hooks"
VENV_PYTHON="$HOME/.engram/venv/bin/python3"
...
ln -sfn "$ENGRAM_HOOKS_DIR/review-checklists" "$HOME/.engram/review-checklists"
