...
**Resolution**: #6's own acceptance criteria ("if no: document the finding and file the gap as an upstream Hindsight feature request") are both satisfied. Filed as a scoped, evidence-backed feature request — exposing `RECALL_STRATEGY_BOOSTS`-equivalent control as a per-request field, not asking for a from-scratch multi-hop rewrite — rather than the vaguer ask in #6's original text. #6 closed in this repo; no local code change is possible until/unless upstream ships something.
