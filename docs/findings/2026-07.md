...
- `koku-cocoindex-flows.py` / `koku-cocoindex-search.py` (new files,
  symlinked into `~/.engram/`): three CocoIndex apps — `koku-docs` (the
...
sweep already had) but in its own file — `~/.engram/logs/cocoindex-transcript-watermarks.json`,
...
**Deployed**: `~/.engram/cocoindex-flows.py` is a symlink to this repo's
...
`~/.engram/logs/cocoindex-transcript-watermarks.json` was created on
...
(`~/.engram/logs/cocoindex-stderr.log`) fed by a single global issues
...
proof**: installed `uv`, ran `uv venv ~/.engram/venv --python 3.14` as
...
`~/.engram/venv/bin/python3 nightly-learn.py --mode hourly` for real: it
...
`uv venv ~/.engram/venv --python 3.14`, a **hermetic, host-independent**
...
`shred -u`'d, `~/.engram-linux` test data directory removed, temporary
...
`~/.engram/logs/fallback-retained.jsonl` with the same tmp-then-rename atomicity as
...
or not — is appended to an audit log (`~/.engram/logs/content-classification-audit.jsonl`) for
...
2. **`cocoindex-flows.py`'s `process_transcript()` never got the `retain_windows()` project-tagging
   ...
   (confirmed via the `~/.engram/cocoindex-flows.py` symlink that this is the daemon serving the
...
deleted via a separate one-off script/log (`~/.engram/logs/leaked-secret-purge.jsonl`), and a
...
failure got permanently baked into `~/.engram/logs/correction-cache.json` as `is_correction:
...
1. `spike/classify.py`: `VERTEXAI_LOCATION` default changed `us-central1` → `global` (confirmed
   working for `claude-haiku-4-5@20251001`, matching `~/.engram/config.env`'s working config
   for the same model).
...
**Verification**: `~/.engram/with-config-env.sh env` (via `env -i` with only `HOME`/`PATH` —
...
```
uv pip install --python ~/.engram/venv/bin/python -U cocoindex
...
(`nightly-learn.py`, `run_nightly()`, ~line 1628). Deployed instantly since `~/.engram/nightly-learn.py`
...
`~/.engram/data/instances/hindsight/` — 6.9GB, PG 18 data last written 2026-07-12, not referenced
...
- The code changes themselves were correct: `setdefault()` semantics preserved (a real exported
  ...
  `~/.engram/config.env`, so this leak was isolated to exactly the 4 changed files.
...
`~/.engram/logs/contradictions-pending.jsonl.bak-20260719-001133` before clearing (all 196
...
`~/.engram/` matching the existing pattern. New `launchd/io.vectorize.cocoindex.engram.plist`
...
`engram-cocoindex-search.py`, their `~/.engram/` symlinks, the `engram-docs` bank, and the
...
spike).** Sampled the live `~/.engram/logs/mcp-calls.jsonl` (857 calls, 21 distinct raw server
...
disable without removing code, or delete `~/.engram/model-refresh-state.json` and the call site
...
`~/.engram/venv` (the same venv CocoIndex already runs under). Confirmed by direct test that
...
and `.../nightly.plist` at `~/.engram/venv/bin/python3`, regenerated and reloaded the live
...
scoping fix, which is why the purge count kept growing until the restart. Also found `~/.engram/`
...
the missing symlink, confirmed clean restart in `~/.engram/logs/cocoindex-stderr.log`, then
...
Haiku's real classification to `~/.engram/logs/prefilter-shadow.jsonl`, and gates nothing for
...
> ~/.engram/venv/bin/python -U 'hindsight-api[all]'`) and confirm the
> deadlock stops appearing in `hindsight-stderr.log`.

**Fixes applied**:
1. Upgraded `cocoindex` 1.0.11 → 1.0.16 in `~/.engram/venv` (`uv pip
...
macOS's bundled system Python, pinned at 3.9.6 — not the project's `~/.engram/
...
3. **Upgraded via the documented runbook**: `uv pip install --python
   ~/.engram/venv/bin/python -U 'hindsight-api[all]'`, then reloaded the
